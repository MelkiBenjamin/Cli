...
func extractTarGz(src, dest, expectedName string) {
	...
	for {
		...
		if err != nil {
			log.Fatalf("Erreur tar lors de la lecture du fichier %s : %v", src, err)
		}
		...
		if err != nil {
			log.Fatalf("Erreur de create tar %s : %v", destpath, err)
		}
		...
		if err := outFile.Close(); err != nil {
			log.Fatalf("Erreur fermeture fichier sortie %s : %v", destpath, err)
		}
	...
	log.Printf("Aucun fichier tar correspondant à %q trouvé dans %s", expectedName, src)
...
func handleFile(dest, url, name string) {
	...
	if strings.HasSuffix(url, ".tar.gz") || strings.HasSuffix(url, ".tgz") {
		log.Printf("tar")
		extractTarGz(dest, bin, name)
		if err := os.Remove(dest); err != nil {
			log.Printf("Impossible de supprimer l'archive %s : %v", dest, err)
		}
		return
	}
	...
	if err := os.Chmod(dest, 0755); err != nil {
		log.Fatalf("Erreur chmod %s : %v", dest, err)
	}
}
...
terraform est extrait dans ~/.local/bin/terraform

helm est extrait dans ~/.local/bin/helm puis rendu exécutable

helmify est extrait dans ~/.local/bin/helmify puis rendu exécutable

le chmod est fait au bon endroit

l’archive téléchargée n’est plus écrasée pendant l’extraction

les fonctions de nommage ont été intégrées directement dans les fonctions existantes

Ce que tu vas voir dans ls -alh ~/.local/bin
...
Si tu veux, je peux maintenant te donner une version encore plus propre qui choisit automatiquement le bon fichier dans une archive quand il y a plusieurs entrées, sans dépendre du nom exact attendu.
