#!/usr/bin/env bash
# Minimal, modular installer without logging for Terraform and k3s.

# Immediate visible marker
printf '%s\n' "debut du script"
...
download(){ local url="$1" dest="$2"; curl "${CURL_OPTS[@]}" -o "$dest" "$url"; }
...
install_k3s(){
  ...
  curl "${CURL_OPTS[@]}" -f -o "$bin_tmp" "${base}/k3s" || abort "échec téléchargement k3s"
